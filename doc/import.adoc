...
voxels, with an implicit ordering and distance between each voxels.
...
ImageVis3D can also associate geometry with a volume.  Generally this
must be in the format of a triangle mesh.
...
import such data into ImageVis3D is to resample it onto a regular grid.
The http://www.visitusers.org/[VisIt] software package might be able to
do this resampling for you; a sample Python script which uses VisIt to
do this is given later in this document.
