...
- *Analyze* (`.hdr` extension, with accompanying `.img`): "Analyze" files
...
This format might be misnamed; finding documentation on it has proved
difficult.

The analyze format is a header + data format, similar in concept to a
...
"soup" form (that is, there is no guaranteed ordering, vertices/edges
are duplicated instead of indexed intelligently).

''This block is experimental and its format will likely change!''

Triangle soup is 3-dimensional.  The format for triangle soup is quite
simple:

- First 64-bits: unsigned integer `n_triangles`
- `n_triangles`*3 32-bit IEEE-754 floating point values.

Each group of 3 values represents a single triangle.
