...
- An unsigned, 64-bit integer which gives the version of the UVF file.
  ImageVis3D has been writing out version 2 UVF files since 1.0 was released in
...
- `histogram_size` 8-byte elements following that: the histogram.
...
It is implied that the second dimension is gradient magnitude.
...
- Final `elems_X` x `elems_Y` x 8 bytes: the histogram, with X as the slowest
  moving dimension.
...
store N-dimensions, regularly gridded data.
...
As one might guess, this block stores a collection of triangles, in
"soup" form (that is, there is guarantee that the triangles form a
surface, nor are they written out with any defined order).

Triangle soup is 3-dimensional.  The format for triangle soup is quite
simple.  First, a header gives size information for arrays:

- First 64-bits: unsigned integer `n_vertices`
- Next 64-bits: unsigned integer `n_normals`
- Next 64-bits: unsigned integer `n_texcoords`
- Next 64-bits: unsigned integer `n_colors`
- Next 64-bits: unsigned integer `n_vertex_indices`
- Next 64-bits: unsigned integer `n_normal_indices`
- Next 64-bits: unsigned integer `n_texcoord_indices`
- Next 64-bits: unsigned integer `n_color_indices`

Then we find the data:

- `n_vertices` 32-bit IEEE-754 FP values representing the triangle vertices.
- `n_normals` 32-bit IEEE-754 FP values representing the triangle normals.
- `n_texcoords` 32-bit IEEE-754 FP values representing texture coordinates
- `n_colors` 32-bit IEEE-754 FP values representing color information

and finally indices into that data:

- `n_vertex_indices` 32-bit unsigned integers indexing into the triangle
vertices.
- `n_normal_indices` 32-bit unsigned integers indexing into the normals.
- `n_texcoord_indices` 32-bit unsigned integers indexing into the texture
coordinates.
- `n_color_indices` 32-bit unsigned integers indexing into the array of
colors.
